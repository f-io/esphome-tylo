...
void SAUNA360Component::setup() {
...
#else
  const BaseType_t core_id = 1; // APP core on dual-core parts
#endif
  ...
  xTaskCreatePinnedToCore(
      [](void *ctx) {
        ...
        // Minimum inter-frame gap after panel EOF before we inject (µs)
        static constexpr int MIN_IFG_US = 520;
        
        uint8_t b = 0;
        uint8_t w0 = 0, w1 = 0, w2 = 0, w3 = 0, w4 = 0, w5 = 0;

        for (;;) {
          // Block for next byte straight from the IDF driver
          int n = uart_read_bytes(PORT, &b, 1, portMAX_DELAY);
          if (n != 1)
            continue;

          // 6-byte window to detect (98 40 07 FD E3 9C)
          w0 = w1;
          w1 = w2;
          w2 = w3;
          w3 = w4;
          ...
          if (panel_eof && !self->tx_queue_.empty()) {
            // Ensure > ~1 char-time idle at 19.2k before TX
            esp_rom_delay_us(MIN_IFG_US);

            // Double-check that the line is still idle
            size_t rx_avail = 0;
            (void)uart_get_buffered_data_len(PORT, &rx_avail);
            if (rx_avail == 0) {
              self->send_data_(); // start TX
            }
          }

          // Feed the normal parser
          self->handle_byte_(b);
          self->last_bytes_[0] = self->last_bytes_[1];
          self->last_bytes_[1] = b;
        }
      },
...
void SAUNA360Component::process_light_heater(uint32_t data) {
  ...
  bool heater_state =
      (data & 0x0001C000) == 0x0001C000; // Bit 14-16 heater state
  ESP_LOGI(TAG, "Light: %s, Heater Status: %s", ONOFF(light_state),
           heater_state ? "Heating" : "Standby");
  ;
...
void SAUNA360Component::set_bath_time_number(float value) {
  ...
  if (this->max_bath_temperature_received_hex_) {
    ...
    data |=
        (static_cast<uint32_t>(this->max_bath_temperature_default_ * 18) << 20);
  }
...
void SAUNA360Component::set_bath_temperature_number(float value) {
  float current_set_temp =
      ((this->setpoint_temperature_received_hex_ & 0x00007FF) / 9.0);
